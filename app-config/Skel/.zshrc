...
# Custom device name?
if [ -e "$HOME/.customShellName" ] ; then
	customDeviceName=" %B%F{green}$(cat $HOME/.customShellName)"
else
	if [ -e "$PREFIX/etc/zsh/.customShellName" ] ; then
		customDeviceName=" %B%F{yellow}$(cat $PREFIX/etc/zsh/.customShellName)"
	fi
fi
...
if [ "$KDE_SESSION_VERSION" != "" ] ; then
	xDE_TYPE="KDE"
elif [ "$GNOME_SHELL_SESSION_MODE" != "" ] ; then
	xDE_TYPE="GTK"
